...
- **Oral** : la reconnaissance vocale automatique reste approximative pour évaluer la prononciation — utilise-la comme aide (feedback indicatif), pas comme juge. Prévoir un créneau hebdomadaire où *toi* (ou un tuteur) écoutes un enregistrement de 2 minutes, même sans comprendre le fond, pour la fluidité/l'aisance.
...
### Sources alignées par chapitre (pour éviter de tout scanner soi-même)

| Chapitre | Thème | Source recommandée | Pourquoi |
|---|---|---|---|
| H1 — Vrije tijd | Loisirs, vacances, avis | cours2langues.com (pages "les vacances", révision présentation/nourriture/hobbys) | Bilingue FR/NL, conçu pour le secondaire belge |
| H2 — Mooi weer | Météo, vêtements, passé composé | Klascement (thème "het weer") | Fiches et chansons gratuites, niveau adapté |
| H3 — Gezondheid | Santé, habitudes alimentaires | DISK/nt2.nl, thème "Eten" (A1, PDF gratuit) + cours2langues (nourriture) | Vocabulaire structuré avec exemples ; croiser les deux pour la traduction française |
| H4 — Winkelen | Achats, shopping | cours2langues.com (pages "le magasin", "les vêtements", actes de communication achat/vente) | Correspondance directe avec le thème, déjà en FR/NL |
| H5 — Op uitstap | Itinéraire, office du tourisme | cours2langues.com (pages "l'itinéraire", "demander le chemin", avec traduction complète) | Le match le plus précis trouvé, quasi prêt à l'emploi |
| Tous | Révision globale / entraînement CE1D | Cahier "S'entraîner au CE1D — Néerlandais" (Librairie Molière, ~13€) | Non gratuit mais peu coûteux, calibré spécifiquement sur l'examen |
| Tous | Complément large | Wikilivres "Néerlandais dans le secondaire" (135 pages, francophone, collaboratif) | À explorer chapitre par chapitre, gratuit |

⚠️ Le vocabulaire trouvé en ligne ne sera jamais identique mot pour mot à celui du manuel scolaire (probablement Sirocco, Éditions Pelckmans, vu la structure "Hoofdstuk/Databank/Woordspinnen"). Utiliser ces sources pour peupler rapidement `content_items` par thème, puis affiner/compléter au fil de l'été avec les pages réellement scannées du manuel quand le temps le permet — mieux vaut démarrer avec un contenu proche à 90% que d'attendre un contenu parfait.

### Ressources générales complémentaires
- **Wablieft** (wablieft.be) : actualités en néerlandais simplifié, bon pour la compréhension écrite/orale de niveau A2.
- **VRT NWS / Karrewiet** : JT néerlandais pour enfants, débit lent — utile pour l'écoute sans décourager.
- **Duolingo** : complément ludique pour le vocabulaire de base, ne couvre pas le programme belge spécifique — en appoint uniquement.
